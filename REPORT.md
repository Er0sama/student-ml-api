...
Parts that are demonstrated by the repository itself rather than by prose are
listed here with their location. The remaining parts have their own sections
below.

| Part | Requirement | Where |
|---|---|---|
| 1 | Application with `/health` and `/predict` | `app.py` |
| 2 | Automated tests, at least four | `tests/test_app.py`, five tests |
| 3 | Feature branches and meaningful commits | Branches `feature/prediction-api`, `feature/model-metadata`; history of PRs #1 and #2 |
| 4 | Professional pull request descriptions | PRs #1, #2, #3, #4 |
| 5 | CI workflow on pull requests | `.github/workflows/ci.yml` |
| 9 | Production-oriented Dockerfile | `Dockerfile`, `.dockerignore` |
| 12 | Container registry | `ghcr.io/er0sama/student-ml-api`, public |
| 13 | Git tag and release version | Tags `v1.0.0`, `v1.1.0` |
| 14 | Automated release workflow on tags | `.github/workflows/release.yml` |
| 15 | Release pipeline stages | Same file; runs `34440046729`, `34440414643` |
| 18 | Version 1.1.0 via the full PR workflow | PR #2, merge commit `a85bb7c` |
